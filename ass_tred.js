...
const getData_binance = async () => {
  ...
  let VIDY 
  if (vidy_idr.buy / usdt_idr.last <= VIDYUSDT)  (VIDY = `${((vidy_idr.buy / usdt_idr.last)*1000000).toFixed()}   ${((vidy_idr.sell / usdt_idr.last)*1000000).toFixed()}`)
  if (vidy_idr.sell / usdt_idr.last >= VIDYUSDT)  (VIDY = `${((vidy_idr.buy / usdt_idr.last)*1000000).toFixed()}   ${((vidy_idr.sell / usdt_idr.last)*1000000).toFixed()}`)
  if (vidy_idr.buy / usdt_idr.last > VIDYUSDT)  (VIDY = `<${((vidy_idr.buy / usdt_idr.last)*1000000).toFixed()}   ${((vidy_idr.sell / usdt_idr.last)*1000000).toFixed()}`)
  if (vidy_idr.sell / usdt_idr.last < VIDYUSDT)  (VIDY = `${((vidy_idr.buy / usdt_idr.last)*1000000).toFixed()}   >${((vidy_idr.sell / usdt_idr.last)*1000000).toFixed()}`)
...
// BTC/IDR : ${(btc_idr.last).toString()}
// USDT/IDR : ${(usdt_idr.last).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}
// VIDY/IDR : ${+vidy_idr.buy}     ${+vidy_idr.sell}
// VIDY/USDT : ${((VIDYUSDT)*1000000).toFixed()}

// Result:
// VIDY : ${VIDY}  AOA : ${(((((0.3 * BTC * 0.9975) / AOA) * 0.9975 - 80) * aoa_idr.buy * 0.997) / btc_idr.last - 0.0005).toFixed(3)}   ${(((((((250000000 / aoa_idr.sell) * 0.997 - 3000) * AOA * 0.9975) / BTC) * 0.9975 - 0.001) * btc_idr.last) / 1000000).toFixed()}
//VIDYIDR : ${+VIDYIDR}
}
...
bot.command('p', async (ctx) => {
  if (ctx.message.chat.id == 797951802) {
